  ...
  try:
   for region in update:
	...
	data_str = data_str.replace("'",'"') 
        ...
        '''
        to_be_json = "["
        to_be_json += "'cnt':"+str(cnt)+","
        to_be_json += "'shp_index':"+str(shp_index)+",'data':"
        for cluster in data:
		to_be_json += str(cluster) + ","
	to_be_json = to_be_json[:-1] + "]"
        json_data = json.dumps(to_be_json).replace("'","''")
        json_update += json_data + "],"
        #insert = """INSERT INTO map_data(shp_index,cnt,data,time_sent) VALUES ("""+str(shp_index)+","+str(cnt)+",\'"+str(json_data)+"\',current_timestamp)"
        #print insert
        #cur.execute(insert)
        #conn.commit()
        '''
...
def get_update():
  ...
  try:
   interval = 3
   query = '''
select shp_idx, start_time, end_time, msg, service from
(select shp_idx, time as start_time, NULL as end_time, msg, 'a' as service from direct_report_stream WHERE time <= (now()) and time >=(now()-INTERVAL '10 hours')
union
select NULL as shp_idx, start_datetime as start_time, end_time, area as msg, 's' as service from outages WHERE start_datetime<= (now()) and start_datetime>= (now()-INTERVAL '10 hours')
union
select NULL as shp_idx, timestamp_ms as start_time, NULL as end_time, text as msg, 't' as service from twitter_stream WHERE timestamp_ms<= (now()) and timestamp_ms>= (now()-INTERVAL '10 hours')
union
select NULL as shp_idx, time_created as start_time, NULL as end_time, message as msg, 'f' as service from facebook_stream WHERE time_created<= (now()) and time_created>= (now()-INTERVAL '10 hours')) as time_chunk
ORDER BY start_time
'''
   cur.execute(query)
   conn.commit()
   ...
   outages = []
   for row in rows:
    if (row[4] == 'a'): #HACK FOR NOW TODO REMOVE 
	continue
    try:
     shp_idx = str(row[0])
